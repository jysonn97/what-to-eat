export default async function fetchPlaceDetails(placeId) {
  try {
    ...
    const rawReviews = result.reviews?.map((r) => r.text).filter(Boolean).slice(0, 10) || [];

    const vibeKeywords = ["romantic", "quiet", "cozy", "noisy", "loud", "crowded", "casual", "upscale", "intimate", "good for groups", "perfect for couples"];
